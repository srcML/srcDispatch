...
protected:
    std::any DataInner() const { return std::make_shared<WhileData>(data); }
...
private:
    ...
    void CollectConditionHandlers() {
        ...
        openEventMap[ParserState::condition] = [this](srcSAXEventContext& ctx) {
            if(!whileDepth) return;
            if((whileDepth + 1) != ctx.depth) return;
